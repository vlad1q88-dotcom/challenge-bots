...
Чтобы он поднимался сам при входе в систему и работал без открытого окна:

1. В папке бота создай файл `start-bot.vbs` со строками:

   ```vbs
   Set shell = CreateObject("WScript.Shell")
   shell.CurrentDirectory = "C:\Users\ИМЯ\Documents\challenge-bots\pushup-bot"
   shell.Run "cmd /c npm start", 0, False
   ```

   Путь замени на свой; `0` означает «без окна».

2. Открой **Планировщик заданий** (Win → «Планировщик заданий») → справа
   **Создать задачу** (не «простую задачу»).
3. Вкладка **Общие**: имя `pushup-bot`, отметь «Выполнять с наивысшими правами».
4. Вкладка **Триггеры** → **Создать** → «При входе в систему».
5. Вкладка **Действия** → **Создать** → Программа: `wscript.exe`,
   Аргументы: `"C:\Users\ИМЯ\Documents\challenge-bots\pushup-bot\start-bot.vbs"`.
6. Вкладка **Условия**: сними галочку «Запускать только при питании от сети»,
   если это ноутбук.
7. Вкладка **Параметры**: отметь «При сбое перезапускать через 1 минуту».

Проверить: перезагрузи компьютер и напиши боту `/help`. Остановить —
в Диспетчере задач завершить процесс `node.exe`.
