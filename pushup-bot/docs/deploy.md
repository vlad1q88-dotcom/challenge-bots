...
Скопируй `scripts/start-bot.vbs.example` в папку бота под именем `start-bot.vbs`
и замени в нём путь на свой:

```vbs
folder = "C:\Users\ИМЯ\Documents\challenge-bots\pushup-bot"
```

Вывод бота пишется в `bot.log` рядом — по нему видно, поднялся он или упал.
