...
* **Отчёт — скриншот, числа бот читает сам.** Участник присылает скриншот вкладки
  **Week**, бот распознаёт подписи над всеми столбиками недели и переносит значения
  по дням. Ручного ввода количества нет.
* **Повторная отправка не удваивает результат.** Бот не складывает отчёты, а
  синхронизирует дни: значение дня заменяется тем, что показывает приложение.
  Поэтому скриншот можно слать хоть трижды в день, хоть раз в неделю — одним
  кадром за всю неделю.
...
* **Бейджи**: серии 3, 7, 10, 14, 21, 30, 60, 100, 150, 200 и 365 дней подряд,
  «Чемпион», «Финишер», «Лузер», а также 🎂 «Именинник» — за дневную норму,
  выполненную в свой день рождения. Дату бот спрашивает при вступлении — шагом
  после ника, и только после ответа вызов считается принятым. Позже её меняет
  сам участник (`/bday`) либо
  инициатор челленджа за него (`/setbday`) — во втором случае участнику ничего
  не сообщается, а если норма в этот день уже выполнена, бейдж прилетает ему
  сразу вместе с поздравлением.
* **Поздравление с анимацией.** В момент выдачи 🎂 всем участникам уходит
  открытка-гифка с конфетти и серпантином, а сообщение отправляется с
  полноэкранным эффектом Telegram. Гифка рисуется на лету и грузится один раз:
  остальным она пересылается по `file_id`. Рисуются медалями: по команде `/badges`
...
| `/join КОД` | принять вызов: выбрать ник и указать день рождения |
...
| `/setbday КОД НИК 16.09` | инициатор ставит дату участнику — тот об этом не узнает |
...
│  ├─ celebrate.ts       анимированная открытка с конфетти (GIF)
...
1. Скриншот скачивается из Телеграма и готовится к распознаванию: мелкие
   картинки увеличиваются примерно до 1200 px по ширине, а из кадра убирается
   всё цветное — остаётся только текст. Без этого подпись, стоящая вплотную над
   ярким столбиком, слипается с ним и читается как мусор.
2. Дальше картинка уходит в OCR (`tesseract.js`, английские языковые данные
   ставятся из npm-пакета `@tesseract.js-data/eng` — в рантайме сеть не нужна).
   Распознавание занимает около секунды.
3. В словах с координатами ищется ось дней недели (`Mon…Sun` или `Пн…Вс`).
4. Для каждой колонки берётся ближайшая подпись над ней. Число принимается, только
   если рядом с ним нет слов, — так строка `1 set · 5 reps total` и счётчик из шапки
   профиля в отчёт не попадают.
5. Заголовок периода (`Aug 31 - Sep 6`) даёт начало недели, и дни недели
   превращаются в конкретные даты. Если заголовок не распознан, берётся текущая
   неделя.
6. Значения переносятся в челлендж: новый день добавляется, изменившийся —
   обновляется, совпавший остаётся как есть. Дни до старта, после финиша и будущие
   пропускаются.

Бот отказывается принимать скриншот, если не нашёл недельный график или ни одного
столбика с числом, — и в ответе пишет, что именно прислать.
...
* **Отчёты синхронизируются, а не накапливаются.** Скриншот вкладки Week — это
  состояние недели целиком, поэтому бот приводит дни к тому, что показывает
  приложение. Отсюда свобода в частоте отправки и невозможность двойного зачёта.
...
`docs/avatar-icon.png` собирается из готовой картинки:
`node scripts/make-avatar-from-icon.ts <путь-к-картинке>` — скрипт вырезает с неё
белую фигуру, обводит чёрным контуром и ставит на фон из столбиков; исходник
в репозиторий не кладётся, только результат.

Остальные варианты лежат в `docs/avatar-*.png` и перерисовываются командой
`node scripts/make-avatar.ts`: `avatar-d` — силуэт в упоре лёжа внутри кольца из
трёх цветов бордов (рисуется кодом, ничего чужого не заимствовано),
`avatar-a…c` — варианты со столбиками. Ставится вручную через
@BotFather: `/setuserpic` → выбрать бота → отправить картинку (Bot API менять
аватар бота не умеет).

## Шрифт

Картинки рисуются шрифтом DejaVu Sans, который лежит в `assets/fonts/`
и регистрируется в canvas явно (`src/render/fonts.ts`). Рядом лежит вырезка из
Noto Color Emoji (26 КБ, только нужные символы) — ею рисуются цветные эмодзи
бейджей: «Чемпион» 🏆, «Финишер» 🏅, «Лузер» 👎 и «Именинник» 🎂. Полагаться на системный
