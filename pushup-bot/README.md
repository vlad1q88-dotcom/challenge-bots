...
3. Для каждой колонки берётся ближайшая подпись над ней. Число принимается, только
   если рядом с ним нет слов, — так строка `1 set · 5 reps total` и счётчик из шапки
   профиля в отчёт не попадают.
4. Заголовок периода (`Aug 31 - Sep 6`) даёт начало недели, и дни недели
   превращаются в конкретные даты. Если заголовок не распознан, берётся текущая
   неделя.
5. Значения переносятся в челлендж: новый день добавляется, изменившийся —
